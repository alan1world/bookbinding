...
def convert_text(myinputfile):
    ...
    myheadtext = textwrap.dedent(r'''    \documentclass[12pt,twoside,openany]{book}
    ...
    \newcolumntype{L}{>{\hspace{0.5em}\hangindent=1.8em\raggedright\arraybackslash}b{6.6cm}} % from array
    % L should be 6.4 for a right of 3.1
    \newcolumntype{R}{>{\raggedleft\arraybackslash}p{3.2cm}} % from array
    \newcolumntype{T}{>{\raggedright\arraybackslash}b{3.1cm}} % from array
    \newcolumntype{F}{>{\hspace{-0.5em}\hangindent=1.8em\raggedright\arraybackslash}b{22ex}} % from array
    \newcolumntype{M}{>{\centering\arraybackslash}m{14ex}}
    \newcolumntype{G}{>{\hspace{-0.6em}\hangindent=1.8em\raggedright\arraybackslash}b{20ex}} % from array
    \usepackage{titlesec}
    \titleformat{\subsection}
    {\normalfont\centering\large\bfseries}{\subsubsection}{0em}{\vspace{3ex}}[\vspace{-1ex}]
    \titleformat{\subsubsection}
    {\normalfont\itshape\centering}{\subsubsection}{0em}{\vspace{-1ex}}[\vspace{-1ex}]

    \usepackage{enumitem}
    \setlist[description]{leftmargin=0.5cm,labelindent=0cm, font=\normalfont\scshape} % font=\small}
...
def reg_cleanup(mytext):
    ...
    # mytext = p.sub(r'}', mytext)
    mytext = p.sub(r'', mytext)
    p = re.compile(r'\\hypertarget{section[0-9-]*}{%', re.MULTILINE)    
    mytext = p.sub(r'', mytext)    
...
def main():
    ...
    # TODO: Figure out a better way of handling title and author - maybe dialog?
